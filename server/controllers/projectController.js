...
exports.updateProject = (req, res) => {
  ...
  connection.query(
    updateProjectQuery,
    [projName, projStatus, req.params.projectID],
    (err, result) => {
      if (err) {
        console.error(err);
        res.status(500).json({ error: "Internal Server Error" });
      ...
      console.log(
        `${result.affectedRows} row updated.ID: ${req.params.projectID}`
      );
      res.status(200).json(result);
    }
  );
...
//       result.forEach((row) => {
//         connection.query(
//           updateProjectPassedDeadlineQuery,
//           [row.projID],
//           (err, result) => {
//             if (err) {
//               console.log(err);
...
WHERE p.projCreatorID = ? `;
  ...
  connection.query(getProjectsQuery, [userId], (err, result) => {
    // Pass the userId to the query
    if (err) {
      console.error(err);
      res.status(500).json({ error: "Internal Server Error" });
      return;
